...
# Etiket kodlaması
df['NObeyesdad'] = label_encoder1.fit_transform(df['NObeyesdad']) 

list = ['Gender','family_history_with_overweight','FAVC','CAEC','SMOKE','SCC','MTRANS','CALC']

main = pd.concat([df.drop('NObeyesdad', axis=1), testdata])

for item in list:
    label_encoder.fit(main[item])
    testdata[item] = label_encoder.transform(testdata[item])
...
testdata = pd.DataFrame(TestData, columns=testdata.columns)

# Özellik seçimi
X,df_new = select_features(X,y,14)

cols_index = [testdata.columns.get_loc(col) for col in df_new.columns]
df_new = testdata.iloc[:, cols_index]
...
y_test = label_encoder1.inverse_transform(y_test)

# Karışıklık matrisi ve sınıflandırma raporu gösterimi
print(confusion_matrix(y_pred, y_test))
print(classification_report(y_pred, y_test))

y_pred_probs = model.predict(df_new)
y_pred = np.argmax(y_pred_probs, axis=1)
y_pred = label_encoder1.inverse_transform(y_pred)
